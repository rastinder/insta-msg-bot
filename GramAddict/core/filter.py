...
FIELD_blacklist_usernames = "blacklist_usernames"
FIELD_MANDATORY_WORDS1 = "mandatory_words1"
...
class SkipReason(Enum):
    ...
    # FIELD_blacklist_usernames = auto()
...
class Filter:
    ...
    def __init__(self, storage=None):
        ...
        if self.conditions is not None:
            logger.info(
                "-" * 70, extra={"color": f"{Fore.YELLOW}{Style.BRIGHT}"})
            logger.info(
    ...
    def is_num_likers_in_range(self, likes_on_post: str) -> bool:
        if self.conditions is not None and likes_on_post is not None:
            if likes_on_post == -1:
                ...
                field_max_likers = self.conditions.get(
                    FIELD_MAX_LIKERS, 1000000)
    ...
    def check_profile(self, device, username):
        ...
        """
        if self.conditions is not None:
            field_skip_business = self.conditions.get(
                FIELD_SKIP_BUSINESS, False)
            field_skip_non_business = self.conditions.get(
                FIELD_SKIP_NON_BUSINESS, False
            )
            field_skip_following = self.conditions.get(
                FIELD_SKIP_FOLLOWING, False)
            field_skip_follower = self.conditions.get(
                FIELD_SKIP_FOLLOWER, False)
            ...
            field_min_potency_ratio = self.conditions.get(
                FIELD_MIN_POTENCY_RATIO, 0)
            field_max_potency_ratio = self.conditions.get(
            ...
            field_mandatory_words1 = self.conditions.get(
                FIELD_MANDATORY_WORDS1, [])
            field_blacklist_usernames = self.conditions.get(
                FIELD_blacklist_usernames, [])
            field_specific_alphabet = self.conditions.get(
                FIELD_SPECIFIC_ALPHABET)
            field_bio_language = self.conditions.get(FIELD_BIO_LANGUAGE)
            field_bio_banned_language = self.conditions.get(
                FIELD_BIO_BANNED_LANGUAGE)
            field_min_posts = self.conditions.get(FIELD_MIN_POSTS)
            field_mutual_friends = self.conditions.get(
                FIELD_MUTUAL_FRIENDS, -1)
            field_skip_if_link_in_bio = self.conditions.get(
                FIELD_SKIP_IF_LINK_IN_BIO, False
            )
            field_skip_if_private = self.conditions.get(
                FIELD_SKIP_PRIVATE, False)
            field_skip_if_public = self.conditions.get(
                FIELD_SKIP_PUBLIC, False)
        profile_data = self.get_all_data(device)
        # print(profile_data.biography)
        for w in field_blacklist_usernames:
            if w.lower() in username.lower() or w.lower() in profile_data.fullname.lower():
                logger.info(
                    "This is a boy profile, skip.",
        ...
        cleaned_biography = " ".join(
            emoji.demojize(
                profile_data.fullname + profile_data.biography.replace("\n", "").replace(".", "").replace("_", ""))
            .lower()
            .split()
        )
        ...
        if (
            ...
            if len(field_mandatory_words) > 0:
                logger.debug(
                    "Checking if account has mandatory words in biography...")
                mandatory_words = []
                for w in field_mandatory_words:
                    if w.casefold() in username.casefold().replace(".", "").replace("_", "") or w.casefold() in cleaned_biography.casefold():
                        mandatory_words.append(w)
                        break
                mandatory_words1 = []
                for w in field_mandatory_words1:
                    if w.casefold() in username.casefold().replace(".", "").replace("_", "") or w.casefold() in cleaned_biography.casefold():
                        mandatory_words1.append(w)
                        break
                if mandatory_words == [] or mandatory_words1 == []:
                    logger.info(
                    ...
                    print(f" words = {mandatory_words}, {mandatory_words1}")
    ...
    def can_pm_to_private_or_empty(self) -> bool:
        ...
        field_pm_to_private_or_empty = self.conditions.get(
    ...
    def get_all_data(self, device):
        ...
        if not is_restricted:
            profile = Profile(
                ...
                follow_button_text=self._get_follow_button_text(
                    device, profileView),
                is_restricted=is_restricted,
                is_private=self._is_private_account(device, profileView),
                has_business_category=self._has_business_category(
                    device, profileView),
    ...
    def _get_followers_and_followings(
        ...
        profileView = ProfileView(
            device) if profileView is None else profileView
        try:
            followers = profileView.getFollowersCount()
        except Exception as e:
            logger.error(
                f"Cannot find followers count view, default is {followers}.")
        ...
        try:
            ...
            logger.error(
                f"Cannot find followings count view, default is {followings}.")
            logger.debug(f"Error: {e}")
