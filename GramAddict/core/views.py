...
class SearchView:
    ...
    def _getSearchEditText2(self):
        for _ in range(2):
            obj = self.device.find(
                resourceIdMatches=case_insensitive_re(
                    ResourceID.ROW_SEARCH_EDIT_TEXT
                ),
            )
            if obj.exists(Timeout.LONG):
                return obj
    ...
    def filter_target(self, target: str, job: str) -> bool:
        logger.info(f"filter {target}")
        search_edit_text = self._getSearchEditText2()
        if search_edit_text is not None:
            logger.debug("Pressing on searchbar.")
            search_edit_text.click(sleep=SleepTime.SHORT)
            search_edit_text.set_text(
